...
class FailingSettledRelay extends MemoryRelay {
  ...
  async publish(event: SignedNostrEvent): Promise<void> {
    if (
      !this.failedOnce &&
      event.tags.some(
        (t) => t[0] === "t" && (t[1] === "settled" || t[1] === "refunded"),
      )
    ) {
...
describe("PactAgent end-to-end workflow integration", () => {
  ...
  describe("economic retry and publication recovery", () => {
    it("coordinator retry after publication failure does not repeat Cashu spending", async () => {
      const failingRelay = new FailingSettledRelay();
      ...
      const rootEvent = s.relay.events.find(
        (e) =>
          e.kind === PACTAGENT_SERVICE_AGREEMENT_EVENT_KIND &&
      ...
      expect(escrowKey).toBeDefined();
      const storedEscrow = (await s.store.read(escrowKey!)) as {
        reference: string;
        revision: number;
        state: string;
      };
  ...
  describe("adversarial integration", () => {
    it("rejects an otherwise valid transition signed by an unauthorized identity", async () => {
      ...
      const tipEvent = history.transitions.at(-2)!.event;

      expect(() =>
        createPactAgreementTransition({
          context,
          history: history.transitions.slice(0, -1).map((t) => t.event),
          predecessorEventId: tipEvent.id,
          nextState: "refunded",
    ...
    it("rejects stale or forked history before Cashu execution", async () => {
      ...
      const alternateTransition = createPactAgreementTransition({
        context,
        history: history.transitions
          .filter((t) => t.content.state !== "refund_authorized" && t.content.state !== "refunded")
          .map((t) => t.event),
        ...
        actorRole: "requester",
        reasonCode: "timeout",
        createdAt: s.clock.now() + 200,
      });
      const signedAlternate = await s.identities.requesterSigner.sign(alternateTransition.event);
    ...
    it("workflow error serialization contains no private material", async () => {
      ...
      const error = new PactAgentWorkflowError(
        "execution_failed",
        "Document summary execution failed: provider_error",
      );
      ...
      expect(error.toJSON()).toEqual({
        ...
        message: "Document summary execution failed: provider_error",
      });
    }, 30_000);
